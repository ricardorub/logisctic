...
def create_compra():
    ...
    if existing_item and existing_item.sku:
        ...
        while True:
            ...
            if not Compra.query.filter_by(sku=sku).first() and not Inventario.query.filter_by(sku=sku).first():
                break
...
def get_available_compra_ids():
    ...
    # Query to get all compra_id and nombre_producto from Compra that are not in the subquery
    available_compras = db.session.query(Compra.compra_id, Compra.nombre_producto)\
