...
def laboratorio_page_controller():
    if 'user_id' not in session:
        return redirect(url_for('auth.login'))
    ...
    return render_template('laboratorio.html', user=user)

def extract_critical_data_controller():
    try:
        data = request.get_json()
        critical_data = data.get('critical_data', [])
        
        # Guardar en la sesión del usuario
        session['lab_data'] = critical_data
        
        return jsonify({'success': True, 'message': 'Datos críticos almacenados.'})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)})

def get_lab_data_controller():
    lab_data = session.get('lab_data', [])
    return jsonify({'success': True, 'data': lab_data})

def apply_deep_learning_controller():
    try:
        lab_data = session.get('lab_data', [])
        if not lab_data:
            return jsonify({'success': False, 'message': 'No hay datos en el laboratorio.'})

        df = pd.DataFrame(lab_data)
        
        # Lógica de optimización con el modelo
        model_input_cols = ['tiempo_entrega', 'stock', 'ventas', 'duracion', 'numero_pedidos']
        
        # Asegurarse que las columnas de entrada del modelo existan
        for col in model_input_cols:
            if col not in df.columns:
                 # Si no existe, intenta buscar una columna con un nombre similar o la crea con un valor por defecto
                 alt_col_name = col.replace('_', ' ')
                 if alt_col_name in df.columns:
                     df[col] = df[alt_col_name]
                 else:
                    return jsonify({'success': False, 'message': f'Falta la columna requerida para el modelo: {col}'})
        ...
        def sane_round(arr):
            ...
            return np.maximum(arr, 0.0).round(2)

        df['tiempo_entrega'] = sane_round(y_pred[:, 0])
        df['cobertura'] = sane_round(y_pred[:, 1])
        df['frecuencia'] = sane_round(y_pred[:, 2])

        # Recalcular el estado final basado en los nuevos valores optimizados
        df['Tiempo de entrega (Días)'] = df['tiempo_entrega']
        df['Cobertura (Días)'] = df['cobertura']
        df['Frecuencia de pedidos por SKU (Días)'] = df['frecuencia']
        
        # Aplicar clasificaciones y calcular el nuevo estado
        clasif_tiempo = df['Tiempo de entrega (Días)'].apply(classify_tiempo_entrega)
        df['Clasificación Tiempo Entrega'] = clasif_tiempo.apply(lambda x: x[0])
        ...
        return jsonify({'success': True, 'optimized_data': optimized_data})

    except Exception as e:
        print(f"[ERROR] apply_deep_learning_controller: {e}")
        return jsonify({'success': False, 'message': str(e)})
