...
def upload_file_controller():
    ...
    for file in files:
        if file and allowed_file(file.filename):
            try:
                ...
                processed_files.append(saved_filename)
                os.remove(file_path)
                
            except Exception as e:
                errors.append(f'Error al procesar el archivo {file.filename}: {str(e)}')
        else:
    ...
    return jsonify({
        ...
        'redirect_url': url_for('predic.view_processed_files_controller')
    })
...
def view_processed_files_controller():
    if 'user_id' not in session:
        return redirect(url_for('auth.login'))
    ...
    if not json_files:
        return redirect(url_for('predic.file_page_controller'))

    latest_timestamp = max([f.split('_')[0] for f in json_files])
    
    for filename in [f for f in json_files if f.startswith(latest_timestamp)]:
        file_path = os.path.join(data_folder, filename)
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
                all_data.extend(data.get('processed_data', []))
                if not all_columns:
                    all_columns = data.get('columns', [])
                filenames.append(data.get('original_name', ''))
        except Exception as e:
            print(f"[ERROR] No se pudo cargar {file_path}: {e}")
            continue
