...
from model.models import Compra, Inventario, Reposicion, Test_compra, Test_Inventario, Test_Reposicion
...
def import_reposiciones():
    try:
        db.session.query(Test_Reposicion).delete()
        
        reposiciones_to_import = Reposicion.query.order_by(Reposicion.id.desc()).limit(40).all()
        
        for reposicion in reposiciones_to_import:
            new_test_reposicion = Test_Reposicion(
                reposicion_id=reposicion.reposicion_id,
                sku=reposicion.sku,
                nombre_producto=reposicion.nombre_producto,
                cantidad=reposicion.cantidad,
                tienda=reposicion.tienda,
                fecha_reposicion=reposicion.fecha_reposicion,
                fecha_despacho=reposicion.fecha_despacho
            )
            db.session.add(new_test_reposicion)
            
        db.session.commit()
        return jsonify({'message': '40 reposiciones importadas exitosamente'}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({'error': str(e)}), 500
