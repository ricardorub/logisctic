...
def predict_api_controller():
    # Helper function to safely load InputLayer with batch_shape (TF 2.12+ compatibility)
    def fixed_input_layer_from_config(config):
        config = config.copy()
    ...
    # Paths to model and preprocessing assets
    modelo_path = 'modelo/modelo_logistica_optima.h5'
    proveedor_encoder_path = 'modelo/proveedor_encoder.pkl'
    ventas_mean_path = 'modelo/ventas_mean.npy'
    ventas_std_path = 'modelo/ventas_std.npy'

    try:
        # Load model with custom object to handle InputLayer deserialization
        modelo = tf.keras.models.load_model(
            modelo_path,
            custom_objects={'InputLayer': fixed_input_layer_from_config},
            compile=False  # Not needed for inference; avoids optimizer issues
        )

        # Load preprocessing assets
        with open(proveedor_encoder_path, 'rb') as f:
            proveedor_encoder = pickle.load(f)
        ventas_mean = np.load(ventas_mean_path)
        ventas_std = np.load(ventas_std_path)
        ...
        if proveedor not in proveedor_encoder.classes_:
            return jsonify({'success': False, 'message': f'Proveedor "{proveedor}" no reconocido por el modelo'})

        ventas_scaled = (ventas - ventas_mean) / ventas_std
        proveedor_encoded = proveedor_encoder.transform([proveedor])[0]

        # Make prediction
        prediction_input = np.array([[ventas_scaled, proveedor_encoded]], dtype=np.float32)
        prediction_output = modelo.predict(prediction_input)[0]

        # Format output
        stock_recomendado = int(round(prediction_output[0]))
        cobertura_estimada_dias = int(round(prediction_output[1]))
        num_pedidos_recomendado = int(round(prediction_output[2]))
        frecuencia_estimada_pedidos_por_dia = float(prediction_output[3])

        return jsonify({
            'success': True,
            'stock_recomendado': stock_recomendado,
            'cobertura_estimada_dias': cobertura_estimada_dias,
            'num_pedidos_recomendado': num_pedidos_recomendado,
            'frecuencia_estimada_pedidos_por_dia': round(frecuencia_estimada_pedidos_por_dia, 2)
        })
