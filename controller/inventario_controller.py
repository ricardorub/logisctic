...
def create_inventario():
    ...
    # Busca si ya existe un item de inventario con el mismo SKU
    inventario_item = Inventario.query.filter_by(sku=compra.sku).first()

    if inventario_item:
        # Si existe, actualiza el item existente
        inventario_item.cantidad_total += compra.cantidad_proveedor
        inventario_item.precio_compra = compra.precio_unitario
        inventario_item.precio_venta = float(precio_venta)
        inventario_item.fecha_recepcion = datetime.now().date()
        inventario_item.compra_id = compra_id
    else:
        # Si no existe, crea un nuevo item de inventario
    ...
    try:
        db.session.commit()
        return jsonify({'message': 'Item de inventario creado/actualizado exitosamente', 'id': inventario_item.id}), 201
