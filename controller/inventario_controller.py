...
def get_all_inventario():
    # Paso 1: Obtener la suma total de cantidades por producto
    total_quantities = (
        db.session.query(
            Inventario.nombre_producto,
            func.sum(Inventario.cantidad_total).label('total_quantity')
        )
        .group_by(Inventario.nombre_producto)
        .all()
    )
    
    # Crear un mapa de producto -> cantidad total para fácil acceso
    quantity_map = {item.nombre_producto: item.total_quantity for item in total_quantities}

    # Paso 2: Obtener los detalles del último item de inventario para cada producto
    ...
    latest_items = (
        ...
        .join(latest_items_subquery, Inventario.id == latest_items_subquery.c.latest_id)
        .all()
    )
...
def create_inventario():
    ...
    # Asegúrate de que este `compra_id` no se haya añadido ya al inventario
    existing_inventario = Inventario.query.filter_by(compra_id=compra_id).first()
    ...
    try:
        ...
        db.session.rollback()
        # Considera registrar `e` para depuración
