...
class Inventario(db.Model):
    ...
    compra_id = db.Column(db.Integer, db.ForeignKey('compras.compra_id'), unique=True, nullable=True)
    proveedor = db.Column(db.String(100), nullable=False)
    nombre_producto = db.Column(db.String(100), nullable=False)
...
class Test_compra(db.Model):
    __tablename__ = 'test_compras'
    id = db.Column(db.Integer, primary_key=True)
    compra_id = db.Column(db.Integer, unique=True, nullable=False)
    proveedor = db.Column(db.String(100), nullable=False)
    nombre_producto = db.Column(db.String(100), nullable=False)
    ...
    inventario = db.relationship(
        'Test_Inventario',
        primaryjoin='Test_compra.compra_id == Test_Inventario.compra_id',
        foreign_keys='[Test_Inventario.compra_id]',
        backref='test_compra',
        cascade='all, delete',
        uselist=False
    )
    ...
    def __repr__(self):
        return f'<Test_compra {self.id}>'


class Test_Inventario(db.Model):
    __tablename__ = 'test_inventario'
    id = db.Column(db.Integer, primary_key=True)
    compra_id = db.Column(db.Integer, db.ForeignKey('test_compras.compra_id'), unique=True, nullable=True)
    proveedor = db.Column(db.String(100), nullable=False)
    nombre_producto = db.Column(db.String(100), nullable=False)
    sku = db.Column(db.String(100), nullable=False)
    precio_compra = db.Column(db.Float, nullable=False)
    ...
    def __repr__(self):
        return f'<Test_Inventario {self.id}>'


class Test_Reposicion(db.Model):
    __tablename__ = 'test_reposicion'
    id = db.Column(db.Integer, primary_key=True)
    reposicion_id = db.Column(db.Integer, unique=True, nullable=False)
    sku = db.Column(db.String(100), nullable=False)
    nombre_producto = db.Column(db.String(100), nullable=False)
    ...
    def __repr__(self):
        return f'<Test_Reposicion {self.id}>'
